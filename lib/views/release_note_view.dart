...
class ReleaseNoteView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_text(context, '版本说明', 'Release Notes')),
      ),
      body: AdaptivePage(
        desktopMaxWidth: 800,
        child: ListView(
          ...
          _buildFeatureCategory(
            context,
            title: _text(context, '同步引擎与架构', 'Sync Engine & Architecture'),
            icon: Icons.sync_rounded,
            features: [
              _FeatureNode(
                _text(context, '分布式 CRDT 协议', 'Distributed CRDT Protocol'),
                _text(
                  context,
                  '基于字段级别的冲突解决基线，确保多端并发修改时的最终一致性。',
                  'Field-first conflict resolution baseline ensuring eventual consistency across multi-node modifications.',
                ),
              ),
              _FeatureNode(
                _text(context, 'HLC 向量时钟', 'HLC Vector Clocks'),
                _text(
                  context,
                  '引入混合同步逻辑时钟（Hybrid Logical Clock），在离线编辑场景下提供确定的事件排序。',
                  'Implementation of Hybrid Logical Clocks to provide deterministic event ordering for offline editing.',
                ),
              ),
              _FeatureNode(
                _text(context, '保险库分库版本管理', 'Vault-Scoped Versioning'),
                _text(
                  context,
                  '针对不同 Vault 独立追踪同步序列，彻底解决多身份切换时的版本号跳动问题。',
                  'Independent sequence tracking per vault to eliminate version jumping during identity switching.',
                ),
              ),
              _FeatureNode(
                _text(context, '增量差分传输', 'Incremental Diffing'),
                _text(
                  context,
                  '仅传输自上次同步以来的变更增量，大幅优化超大账号库下的同步流量消耗。',
                  'High-performance delta transmission ensuring optimal bandwidth usage even in large account libraries.',
                ),
              ),
              _FeatureNode(
                _text(context, '冲突可视化警告', 'Conflict Resolving Banner'),
                _text(
                  context,
                  '自动识别并标记“交错修改”状态，阻止潜在的数据覆盖，保护您的数据安全。',
                  'Visual flagging of interleaved modifications to prevent silent overwrites and ensure data integrity.',
                ),
              ),
              _FeatureNode(
                _text(context, '全量模板同步', 'Template Synchronization'),
                _text(
                  context,
                  '现在支持自定义账号模板的云端同步与多端对齐，确保您的分类体系在所有设备上保持一致。',
          ...
          _buildFeatureCategory(
            ...
            features: [
              _FeatureNode(
                _text(context, '端到端加密 (E2EE)', 'End-to-End Encryption'),
                _text(
                  context,
                  '所有账号负载在离机前完成加密。服务端仅作为加密块的“哑中转站”，无法窥视明文。',
                  'All account payloads are encrypted on-device. Servers act as dumb relay nodes for opaque blocks.',
                ),
              ),
              _FeatureNode(
                _text(context, '保险库主密钥系统', 'Vault Master Key'),
                _text(
                  context,
                  '基于设备绑定的安全根密钥（VMK），为所有加密操作提供底层信任根。',
                  'Device-bound security root providing the foundational trust for all cryptographic operations.',
                ),
              ),
              _FeatureNode(
                _text(context, '局部数据脱敏保护', 'Selective Data Masking'),
                _text(
                  context,
                  '默认对密码、卡号、邮箱等敏感字段进行掩码处理，有效防范侧视泄密。',
                  'Privacy-by-default masking for sensitive fields to prevent over-the-shoulder data leaks.',
                ),
              ),
              _FeatureNode(
                _text(context, 'OS 级安全存储集成', 'Secure Storage Integration'),
                _text(
                  context,
                  '身份令牌与密钥存储于 iOS Keychain / Android Keystore 等底层的安全区域中。',
                  'Keys and identity tokens are persisted within platform-level secure enclaves.',
                ),
              ),
              _FeatureNode(
                _text(context, '匿名节点标识', 'Anonymous Node IDs'),
                _text(
                  context,
                  '随机生成的 Node ID 与 Vault ID，在同步过程中完全隐匿您的物理身份信息。',
                  'Randomly generated identifiers ensuring your physical identity remains untraceable during sync.',
                ),
              ),
              _FeatureNode(
                _text(context, '主密码弹性管理', 'Flexible Master Password'),
                _text(
                  context,
                  '支持随时修改主密码，并允许在初始“跳过设置”后重新启用密码保护，兼顾便捷与安全。',
                  'Modify your master password anytime or enable protection even after skipping initial setup.',
                ),
              ),
              _FeatureNode(
                _text(context, '保险库彻底销毁', 'Universal Factory Reset'),
                _text(
                  context,
                  '在危险区域提供一键重置功能，彻底擦除本地数据库、身份密钥与偏好设置。',
          ...
          _buildFeatureCategory(
            ...
            features: [
              _FeatureNode(
                _text(context, '高密度账号中心', 'High-Density Hub'),
                _text(
                  context,
                  '专为海量账号设计的极简列表引擎，支持丝滑滚动与即时预览。',
          ...
          _buildFeatureCategory(
            ...
            features: [
              _FeatureNode(
                _text(context, '响应式自适应布局', 'Modern Adaptive UI'),
                _text(
                  context,
                  '一套代码完美适配 Mobile、平板与高分屏桌面端，提供原生级的交互体验。',
                  'Fluid responsive design scaling from small mobile screens to professional workstations.',
                ),
              ),
              _FeatureNode(
                _text(context, '实时全局搜索', 'Instant Global Search'),
                _text(
                  context,
                  '针对标题、邮箱及模板类别的极速过滤，让搜索不再有等待感。',
                  'Extreme filtering by titles, emails, or categories with zero latency perception.',
                ),
              ),
              _FeatureNode(
                _text(context, '同步状态指示英雄卡', 'Sync Status Hero'),
                _text(
                  context,
                  '实时显示同步诊断信息、服务偏移及最新版本进度，让技术状态透明化。',
                  'Real-time transparency of sync diagnostics, clock drift, and version milestones.',
                ),
              ),
              _FeatureNode(
                _text(context, '微动效果与视觉语境', 'Micro-Interactions'),
                _text(
                  context,
                  '精心调校的悬停反馈、成功动画与过场动效，营造高级感软件氛围。',
    ...
    return Center(
      child: Column(
        children: [
          ...
          const SizedBox(height: 16),
          Text(
            _text(context, '由 SecretRoy 核心团队呈献', 'Presented by SecretRoy Core Team'),
