...
class MockScenarioData {
  ...
  int get pendingSyncCount => syncChanges.where((c) => c.status == LocalSyncStatus.pendingReview).length;
...
class MockScenario {
  ...
  static MockScenarioData standardUser() {
    final accounts = <AccountItem>[
      ...
      MockDataFactory.secureNote(
        id: 'acc_note',
        name: '助记词备份',
        content: 'abandon ability able about above absent absorb abstract absurd',
      ),
    ...
    final templates = <AccountTemplate>[
      MockDataFactory.template('tpl_social')
          ...
          .addField(key: 'platform', label: '平台', type: AccountFieldType.text, isPrimary: true, isRequired: true)
          .addField(key: 'social_username', label: '用户名', type: AccountFieldType.text, isRequired: true)
          .addField(key: 'social_password', label: '密码', type: AccountFieldType.password, isSecret: true)
          .build(),
      MockDataFactory.template('tpl_server')
          .withTitle('服务器')
          .withCategory(TemplateCategory.custom)
          .addField(key: 'host', label: '主机', type: AccountFieldType.url, isPrimary: true, isRequired: true)
          .addField(key: 'ssh_user', label: 'SSH用户', type: AccountFieldType.text, isRequired: true)
          .addField(key: 'ssh_port', label: '端口', type: AccountFieldType.number)
          .addField(key: 'ssh_key', label: 'SSH密钥', type: AccountFieldType.password, isSecret: true)
          .build(),
    ];
    ...
    final totps = <TotpCredential>[
      ...
      MockDataFactory.totp(id: 'totp_google', label: 'Google')
          .fromOtpAuthUri('otpauth://totp/Google:alice@gmail.com?secret=JBSWY3DPEHPK3PXP&issuer=Google')
  ...
  static MockScenarioData conflictPair() {
    ...
    final fieldConflictLocal = MockDataFactory.account(id: 'acc_field_conflict', name: 'Field Conflict')
    ...
    final fieldConflictRemote = MockDataFactory.account(id: 'acc_field_conflict', name: 'Field Conflict')
    ...
    return MockScenarioData(
      accounts: [pair.local, pair.remote, fieldConflictLocal, fieldConflictRemote],
    );
  }
  ...
  static MockScenarioData largeDataset({
    ...
    final templates = List.generate(templateCount, (i) {
      return MockDataFactory.template('tpl_${_pad(i, 3)}')
          ...
          .withCategory(TemplateCategory.custom)
          .addField(key: 'field1', label: 'Field 1', type: AccountFieldType.text)
          .addField(key: 'field2', label: 'Field 2', type: AccountFieldType.password, isSecret: true)
          .build();
    });
  ...
  static MockScenarioData pendingSync({int pendingCount = 10}) {
    final accounts = MockDataFactory.batchWebsiteAccounts(count: pendingCount, prefix: 'acc');
