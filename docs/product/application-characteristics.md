...
| Vault Health 本地体检 | 把加密、备份、凭据风险和待同步状态变成可见产品能力 | `vault_health_calculator.dart`, `vault_health_view.dart`, `vault_health_report.dart`, `SecureStorageService`, `IdentityService` | 离线可用、不上传、每个风险项指向可执行动作、评分可测试 |
...
当前全量测试基线约 619 个测试用例，88 个测试文件（2026-05-16 更新）。历史基线：187 passed, 1 skipped（Vault Health 合并时）。新增 Vault Health 计算服务单元测试 29 项，覆盖评分边界、弱密码、重复密码、陈旧记录、不完整记录和缺少 2FA 检测。跳过项仍是 Windows runner 下不稳定的 UDP broadcast discovery。

代码质量收敛基线（quality-pass-1, commit daea7fe）已纳入回归：
- `dart analyze lib test` 通过，0 issues。
- `flutter test` 约 619 个测试用例通过；历史基线 187 passed, 1 skipped（Vault Health 合并时）。跳过项仍是 Windows runner 下不稳定的 UDP broadcast discovery。
...
当前 Vault Health（阶段一）测试基线：
- `test/services/vault_health_calculator_test.dart` 29 passed。
- 覆盖评分边界（0/50/70/90/100）、等级映射、弱密码检测、重复密码检测、陈旧记录（>180 天）、不完整记录（缺少 URL）、缺少 2FA 检测。
