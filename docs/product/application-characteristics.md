...
| 2FA/TOTP 验证器 | 在账号内保存网站 TOTP 密钥并本地生成动态验证码 | `totp_service.dart`, `account_edit_view.dart`, `account_list_tile.dart`, `AccountFieldType.totp` | secret 默认隐藏、不搜索、不泄露到服务端、列表只提示已配置、复制验证码而不是 secret、验证码剪贴板 45 秒清理、多设备时间一致性 |
...
- `test/sync/sync_conflict_recovery_test.dart`
- `dart analyze lib test`
- `flutter test`，结果为 100 passed, 1 skipped；跳过项仍是 Windows runner 下不稳定的 UDP broadcast discovery。
...
T11 的展示原则是：账号详情页可以生成和复制当前验证码；列表页和搜索卡片只提示“已配置 2FA”，不展示原始 secret、不显示当前验证码、不提供复制 secret 入口。复制验证码后，剪贴板只在内容仍为刚复制的验证码时自动清理，不能覆盖用户后续复制的新内容。

当前全量测试基线已更新为 100 passed, 1 skipped。如果后续这些长链路测试再次超时，需要优先判断是 Flutter 测试进程权限/生命周期问题，还是同步链路真实阻塞。
...
| 优先级 | Roadmap 项 | 代码依据 | 收敛目标 |
|---|---|---|---|
| P0 | 服务端认证和传输边界 | `roy_server/system/routes/**`, `sync_settings_view.dart` | 弱服务器仍然可以简单，但公网/局域网风险必须可解释，未认证请求不能读写 vault |
| P0 | 解锁与密钥托管 | `BiometricAuthService`, `EnhancedCryptoService`, `SecuritySettingsView` | 生物识别、无密码模式和主密码托管不能让 UI 文案超过真实安全边界 |
| P1 | 同步状态机清理 | `SyncService`, `sync_settings_view.dart` | UI 消费稳定状态，不再猜 transport/protocol/persistence/recovery 的内部细节 |
| P1 | 备份、恢复和导入一致性 | `VaultDumpCoordinator`, `SecureStorageService.replaceAllDataForImport` | 备份可演练，导入失败可回滚，导入后的 sync/outbox 状态可解释 |
| P1 | Vault Health 本地体检 | `SecureStorageService`, `IdentityService`, `SyncService`, password tools | 本地显示加密、备份、恢复、弱/复用/陈旧凭据、缺少恢复数据和待同步风险 |
| P1 | 全局敏感复制策略 | `SensitiveClipboardService`, 全局 `Clipboard.setData` 调用 | 密码、配对码、恢复码和复制全部等高风险复制有统一清理策略 |
| P1 | UI 架构与本地化收敛 | 超大 view/service 文件、`_text(...)` 与 l10n 并存 | 拆分大视图，统一组件和文案路径，避免功能继续堆进单个文件 |
| P2 | 2FA 下一阶段 | `TotpService`, `AccountFieldType.totp`, 2FA 文档 | QR 导入、恢复码模板和设备时间漂移提示在安全/恢复主线稳定后推进 |

执行队列以 `docs/product/iteration-tasks.md` 的 `T9+` 为准；中长期方向以 `docs/todo.md` 为准。
