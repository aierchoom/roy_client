...
> **版本:** v1.0.0
> **状态:** Draft / Proposed
> **核心标签:** `E2EE` `Zero-Knowledge` `CRDT` `Local Vault Identity`

## 1. 摘要与构想愿景 (Abstract)

SecretRoy 旨在成为一款本地优先、端到端加密（E2EE）的跨平台密码管理器。不同于传统的以服务端数据库为核心的“中心化登录系统”，SecretRoy 抛弃了经典的“邮箱+密码”注册体系，转向使用由客户端生成和持有的 **本地 Vault 身份与设备密钥体系**。
...
### 2.1 本地 Vault 身份根
全系统的安全基点是客户端本地生成并保存的 Vault 身份材料。当前实现中，Vault 身份包含：
- **Vault ID**：用于区分不同保险库。
- **设备 ID**：用于区分同一保险库下的不同客户端设备。
- **私钥材料**：用于设备链接、同步身份和后续签名能力。
- **对称密钥材料**：用于保护本地加密数据与导入导出数据包。

这些材料默认留在设备本地的安全存储中。跨设备迁移依赖面对面链接、远程配对或加密恢复码。

### 2.2 密钥树分叉推导 (Key Derivation Tree)
由本地 Vault 身份材料承载密码库业务所需的两类加密原语：
...
多个完成 Vault 链接的设备共同组成多端同步拓扑。我们利用 **基于 HLC 控制的 Git 模型** 来调和它们：
