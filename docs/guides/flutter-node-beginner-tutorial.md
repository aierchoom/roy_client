...
- **本地数据库加密**：`DatabaseFileCipher` 使用 AES-GCM-256 二进制信封加密，长期落盘文件为 `secret_roy_vault.db.enc`
- **主密码验证**：PBKDF2-HMAC-SHA256（100,000 轮）派生包装密钥，保护随机 32 字节数据密钥
- **同步 Payload 加密**：`sroy-sync:` 前缀 + HKDF-SHA256 + AES-256-GCM 信封，服务端仅存储加密 blob，不解析明文
- **身份系统**：`IdentityService` 在首次初始化时自动生成 `deviceId`、`vaultId` 等身份标识，非 mock
- **LAN 配对**：使用 X25519 密钥交换 + 8 位可读字符码（非 6 位数字码）
- **离线恢复码**：`sroy-recovery:` 前缀 + PBKDF2 + AES-GCM-256

但项目仍处于原型阶段，尚未达到可直接投产的安全产品级别。你应该把它理解成：

“一个具备真实加密能力的教学和架构学习项目”，而不是“已经可以放心商用的密码管理器”。
