...
> **状态**: 已部分过时（2026-05-16）。第一阶段改造项一至五已实施。第二阶段历史字段区已部分实施（编辑页可见，列表页 `legacyFieldCount` 已接入）。5 周排期已废止，后续按实际迭代节奏推进。
...
第一阶段改造项一至五已完成。后续工作不再按原 5 周排期执行，改为按实际迭代节奏推进：

- 已实施：合并式保存、模板缺失禁保存、服务层模板删除校验、删字段确认、切模板确认。
- 部分实施：历史字段区（编辑页已支持，列表页 `legacyFieldCount` 已接入）。
- 未实施：编辑器状态模型重构、模板版本与懒迁移机制、切模板迁移向导。
