...
| 内部兼容码 | 无普通用户入口 | P2 | 不推荐用户直接接触 | `sroy-link:` 是内部承载格式，不作为恢复入口展示 |
...
| 加密边界 | 新设备 claim 必须发送临时公钥；主设备只返回 `wrapped_transfer_code` |
...
- claim 必须返回 `wrapped_transfer_code`，不能返回明文 `transfer_code`。
...
- 服务端 approve 路由拒绝 plaintext `sroy-link:` bundle。
- 新设备成功领取后服务端删除 pairing session 和密文 bundle。
- 新设备本地解密后再走导入流程。
...
- approve 上传的 `wrapped_vault_bundle` 必须是 `sroy-pairing:`。
- 服务端必须拒绝 plaintext `sroy-link:` bundle。
- 服务端日志和持久化数据中不能出现可读 vault key material。
- 密文 bundle 成功领取后不能再次领取。
...
- `sroy-link:` 本身是 bearer secret，拿到即可导入 vault key material。
- 如果被当成用户可复制的普通恢复码，会绕过恢复密码和配对审批心智模型。
- 如果服务器中转明文 `sroy-link:`，服务器就有机会读取 vault 密钥材料。

已缓解：

- 普通 UI 不再突出“原始链接码”。
- 远程配对 approve 路由拒绝明文 `sroy-link:` bundle。
- 面对面链接当前必须把内部码加密为 `wrapped_transfer_code` 返回。
...
- 底层内部导入路径只服务配对链路和测试，不作为产品主入口。

安全验收：

- 服务器中转路径不能接受 plaintext `sroy-link:` bundle。
- 面对面 claim 返回不得暴露 plaintext `transfer_code`。
- 文档、UI、错误提示都必须避免把 `sroy-link:` 称为“恢复码”。
...
- 代码级文档可以出现 `sroy-link:`、`sroy-recovery:`、`sroy-pairing:`。
