...
├── models/             # 数据模型：AccountItem、AccountTemplate、Hlc、TOTPCredential 等
├── providers/          # Provider 状态层（EnhancedAppProvider、ThemeProvider）
├── services/           # 业务服务层
│   ├── importers/      # 导入协调器：CSV、Bitwarden JSON、重复检测
...
│   └── ...
├── sync/               # 同步核心：CRDT 合并、Payload 编解码、SyncService（push/pull/conflict）
├── system/             # 系统级子模块（service_manager 的拆分逻辑）
...
├── utils/              # 通用工具与常量
├── views/              # 页面（accounts、home、settings、unlock、templates 等）
└── widgets/            # 可复用组件

test/
├── models/             # 模型序列化与兼容性测试
├── services/           # 加密、存储、身份、TOTP、剪贴板测试
├── sync/               # CRDT、状态机、配对、Payload 编解码测试
└── widgets/            # 当前仅 account_list_tile_test.dart

docs/                   # 项目文档（架构、安全、同步、产品、报告等）
tool/                   # 开发工具脚本
```
...
- `ServiceManager`（`lib/services/service_manager.dart`）是全局单例，负责统筹所有服务的生命周期与解锁状态；新服务接入时应避免继续膨胀其职责。
- 导入顺序：Dart SDK → Flutter SDK → 第三方包 → 项目内部（按相对路径）。
...
- **测试文件数**：24
- **测试用例数**：120+
- **覆盖范围**：models、services、sync、widgets
- **Widget 测试**：极少，目前仅 `account_list_tile_test.dart`
...
| 身份与配对 | `test/services/identity_service_test.dart`、`test/sync/lan_pairing_service_test.dart` |
...
| TOTP | `test/services/totp_service_test.dart`（RFC 6238 标准向量） |
...
- **触发条件**：`main`/`master` 分支的 push、tag（`v*`）、PR。
- **Validate Job**：`flutter pub get` → `dart analyze lib test` → `flutter test`（Ubuntu）。
- **Android Minimal APK**：分 ABI 构建 release APK，上传 artifact（保留 14 天）。
- **Windows Portable**：构建 Windows release，打包为 ZIP，上传 artifact。
...
| 同步核心 | `lib/sync/sync_service.dart`（含 `sync_service_pull.dart`、`push.dart`、`conflict.dart`） |
...
| 导入协调器 | `lib/services/importers/import_coordinator.dart` |
